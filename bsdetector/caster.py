...
def extract_summary_terms(nerkwp_count_tpl_list, comparison_text=None, sort_by="both",
    ...
    for i in range(len(terms)):
        ...
        for t2 in terms:
            ...
            if t1 != t2 and t1_set.issubset(t2_set):
                if t2 in consolidated_tf_dict:
                    if t1 in consolidated_tf_dict:
                        # print("^- updating t2: {} += t1 from both, and del t1: {}".format(t2, t1))
                        consolidated_tf_dict[t2] += int(term_freq_dict[t1]) + consolidated_tf_dict[t1]
                        del consolidated_tf_dict[t1]
                    else:
                        # print("^ updating t2: {} += t1: {}".format(t2, t1))
                        consolidated_tf_dict[t2] += int(term_freq_dict[t1])
                else:
                    t2_foundin_t3 = False
                    for t3 in list(consolidated_tf_dict.keys()):
                        t3_set = set(t3.split())
                        if t2_set.issubset(t3_set):
                            # print("^ updating t3: {} += t2: {}".format(t3, t2))
                            consolidated_tf_dict[t3] += int(term_freq_dict[t2])
                            t2_foundin_t3 = True
                    if t2_foundin_t3:
                        # print("+ adding t2: {} combined with t1: {}".format(t2, t1))
                        consolidated_tf_dict[t2] = int(term_freq_dict[t2]) + int(term_freq_dict[t1])
    for i in range(len(terms)):
        t1 = terms[i]
        if t1 in consolidated_tf_dict:
            continue
        for t2 in terms:
            if t2 in consolidated_tf_dict:
                continue
            t1_set, t2_set = set(t1.split()), set(t2.split())
            if (t1 == t2) or (t1 in t2) or t1_set.issubset(t2_set):
                # print("abouttocontinue {} and {}".format(t1, t2))
                continue
            if is_str_set_subset_of_list_str_sets(t2, list(consolidated_tf_dict.keys())):
                continue
            elif term_freq_dict[t2] >= tf_threshold:
                # print("+ adding t2: {} (t1 was {})".format(t2, t1))
                consolidated_tf_dict[t2] = int(term_freq_dict[t2])

    for key_term in list(consolidated_tf_dict.keys()):
        if consolidated_tf_dict[key_term] < tf_threshold:
            del consolidated_tf_dict[key_term]

    if comparison_text:
        # myTODO: implement string metrics to detect overlapping terms or do fuzzy string matching (c.f. fuzzywuzzy)
